...
# There seems to be a problem in parsing rows 141 and 674. E.g. 674 looks like this:

# 674  0  0  1  0 32  6  2  0  0   0   0   0   0   5   0   6   2 30"

# There are two workaround lines in the R block as a result, but they
...
#+source: analysis
#+begin_src R :var headers=stat-headers :var data=offensive-stats
  # apply the headers to the list
  headers[[18]] <- "Attendance"
  colnames(data) <- headers[1,]
  
  ## The following two lines are required because parsing bugs are
  ## causing corrupt data in these two rows.
  data <- data[-c(141, 674),]
  data$Attendance <- as.integer(data$Attendance)
  
  # perform a simple correlation of each column with the attendence
  corrln <- cor(data[names(data) != "Attendance"], data$Attendance)
